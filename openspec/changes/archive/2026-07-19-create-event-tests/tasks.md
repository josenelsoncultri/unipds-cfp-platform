...
- [x] 1.1 Add/verify Cypress configuration `frontend-e2e/cypress.config.ts` with `baseUrl: 'http://localhost:4200'`.
- [x] 1.2 Configure Cypress TypeScript configuration (`tsconfig.json` or support files) if needed.

## 2. Event Registration E2E Tests Implementation

- [x] 2.1 Create `frontend-e2e/src/e2e/event-registration.cy.ts` (and ensure directory `apps/frontend-e2e/src/e2e/` compatibility if requested).
- [x] 2.2 Implement **Cenário de Sucesso**:
  - `cy.visit('/event/new')`
...
- [x] 2.3 Implement **Cenário de Erro**:
  - `cy.visit('/event/new')`
...
- [x] 3.1 Run Cypress E2E test suite to verify spec passes.
